...
var lazyLoad = {
    ...
    isLoad: function(elem) {
        var cHeight = window.innerHeight || document.body.clientHeight || document.documentElement.clientHeight, //可视窗口的高度
            ...
            threshold = 100, //表示图片在什么时候进行加载，默认为 0 表示当图片进入可视区域立即加载；设为正数表示图片距离 threshold像素进入可视区域进行加载；设为负数表示图片进入可视区域threshold像素时进行加载。
    ...
    loadImg: function(elems, isFadein) {
        for (var i = 0, len = elems.length; i < len; i++) {
            if (this.isLoad(elems[i])) {
                ...
                if (elems[i] && elems[i].className !== "loaded") {
                    //针对使用data-自定义的属性，要使用getAttribute()获取值
                    elems[i].src = elems[i].getAttribute("data-src"); //实际的url替换原来的url
